 ...
 * 自动初始化数据库：
 * 1. 建表（如果不存在）
 * 2. 如果 word_libraries 为空，则插入种子数据
 */
export async function autoInitDatabase(databaseUrl: string) {
  ...
  try {
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS library_words (
        ...
        UNIQUE KEY library_word_idx (library_id, word_id),
    ...
    await conn.execute(`
      CREATE TABLE IF NOT EXISTS word_progress (
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY user_word_library_idx (user_id, word_id, library_id)
      )
    `);

    console.log("[auto-init] Tables ensured.");

    // 2. 检查是否需要 seed
    const [rows] = await conn.execute("SELECT COUNT(*) as cnt FROM word_libraries");
    const count = (rows as any)[0].cnt;

    if (count > 0) {
      console.log(`[auto-init] word_libraries already has ${count} rows, skipping seed.`);
      return;
    }

    console.log("[auto-init] Seeding database...");
    ...
    for (const [name, desc, cat] of libs) {
      await conn.execute(
        "INSERT INTO word_libraries (name, description, category, is_builtin, word_count, created_at, updated_at) VALUES (?, ?, ?, true, 0, NOW(), NOW())",
        [name, desc, cat]
      );
    }

    const [libRows] = await conn.execute("SELECT id, category FROM word_libraries");
    const libMap: Record<string, number> = {};
    for (const r of libRows as any[]) libMap[r.category] = r.id;
    ...
    const allData: { lib: string; words: WordEntry[] }[] = [
      ...
      },
      {
        ...
        words: [
          ...
          ["abstain", "/əbˈsteɪn/", '[{"pos":"v.","meaning":"弃权；戒除"}]', '[{"phrase":"abstain from","meaning":"戒除..."}]', '[{"sentence":"He decided to abstain from voting.","translation":"他决定弃权投票。"}]'],
          ["abuse", "/əˈbjuːz/", '[{"pos":"n./v.","meaning":"滥用；虐待"}]', '[{"phrase":"drug abuse","meaning":"药物滥用"}]', '[{"sentence":"Child abuse is a serious problem.","translation":"虐待儿童是一个严重的问题。"}]'],
    ...
    for (const group of allData) {
      ...
      const flatWords = group.words.flat();
      const placeholders = group.words.map(() => "(?, ?, ?, ?, ?, NOW())").join(",");

      const inserted = await conn.execute(
        `INSERT INTO words (word, phonetic, definitions, phrases, examples, created_at) VALUES ${placeholders}`,
        flatWords
      );
      const firstId = Number((inserted as any)[0].insertId);

      for (let i = 0; i < group.words.length; i++) {
        await conn.execute(
          "INSERT INTO library_words (library_id, word_id, added_at) VALUES (?, ?, NOW())",
          [libId, firstId + i]
        );
      }

      await conn.execute("UPDATE word_libraries SET word_count = ? WHERE id = ?", [
        group.words.length,
        libId,
      ]);
      console.log(`[auto-init] Inserted ${group.words.length} words for ${group.lib}`);
    }
